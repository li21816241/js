	...
	$.fn.nicefileinput = function(options) {
		...
		return this.each(function() {
			...
			if ($(self).attr('data-styled') === undefined) {
				...
				var filename = $('<input type="text" readonly="readonly" class="form-control" style="width:70%">')
					.css({
						'display': 'block',
						'float': 'left',
						'margin': 0,
				...
				if(settings.afterit || '') {
					var afterit = $("<div>")
						...
						.addClass('NFI-button NFI'+guid)
						.html(settings.afterit);
					//wrapper.after(afterit);
				}
